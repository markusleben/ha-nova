...
`);
  ...
  it("does not flag harmless text inspection that only prints a mutating ws JSON example", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        ...
        "item": {
            "id": "cmd_printf",
            "type": "command_execution",
            "command": "printf '{\\"type\\":\\"automation/reload\\"}\\n'",
            "aggregated_output": "{\\"type\\":\\"automation/reload\\"}",
            "exit_code": 0,
            "status": "completed",
        },
    },
...
`);
  ...
  it("fails when collision search/related builds the payload through append redirection before inline jq reuse", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        ...
        "item": {
            "id": "cmd_payload",
            "type": "command_execution",
            "command": ": > entity-payload.json\\nprintf '{\\"type\\":\\"search/related\\",\\"item_type\\":\\"entity\\",\\"item_id\\":\\"light.a\\"}' >> entity-payload.json\\nha-nova relay ws --data-file entity-payload.json --jq-file related-filter.jq --out related.json",
            "aggregated_output": "",
            "exit_code": 0,
            "status": "completed",
        },
...
`);
  ...
  it("does not flag inline related jq when the same command block overwrites a related payload before reuse", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        "type": "item.completed",
        "item": {
            "id": "cmd_projection_then_reuse",
            "type": "command_execution",
            "command": "cat <<'EOF' > payload.json\\n{\\"type\\": \\"search/related\\", \\"item_type\\": \\"entity\\", \\"item_id\\": \\"light.a\\"}\\nEOF\\ncat <<'EOF' > payload.json\\n{\\"type\\": \\"config/entity_registry/get\\", \\"entity_id\\": \\"automation.a\\"}\\nEOF\\nha-nova relay ws --data-file payload.json --jq-file area-projection.jq --out rows.json",
            "aggregated_output": "",
            "exit_code": 0,
            "status": "completed",
        },
...
`);
  ...
  it("does not flag inline related jq when a single shell line overwrites a related payload before reuse", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        ...
        "item": {
            "id": "cmd_projection_then_reuse",
            "type": "command_execution",
            "command": "printf '{\\"type\\":\\"search/related\\",\\"item_type\\":\\"entity\\",\\"item_id\\":\\"light.a\\"}' > payload.json; printf '{\\"type\\":\\"config/entity_registry/get\\",\\"entity_id\\":\\"automation.a\\"}' > payload.json; ha-nova relay ws --data-file payload.json --jq-file area-projection.jq --out rows.json",
            "aggregated_output": "",
            "exit_code": 0,
            "status": "completed",
        },
...
`);
  ...
  it("does not flag inline related jq when an && chain overwrites a related payload before reuse", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        ...
        "item": {
            "id": "cmd_configs",
            "type": "command_execution",
            "command": "while read entity_id; do ha-nova relay core --method GET --path /api/config/automation/config/$unique_id --jq-file config_filter.jq --out config.json; done",
            "aggregated_output": "\\n".join([
                "ITEM[1]=automation.a",
                "UNIQUE_ID[1]=111",
                "ITEM[2]=automation.b",
...
`);
  ...
  it("fails when collision search/related reuses a tee -a payload with an inline jq-file", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        ...
        "item": {
            "id": "cmd_payload",
            "type": "command_execution",
            "command": "printf '{\\"type\\":\\"search/related\\",\\"item_type\\":\\"entity\\",\\"item_id\\":\\"light.a\\"}' | tee -a entity-payload.json >/dev/null\\nha-nova relay ws --data-file entity-payload.json --jq-file related-filter.jq --out related.json",
            "aggregated_output": "",
            "exit_code": 0,
            "status": "completed",
        },
...
`);
  ...
  it("fails when a related payload is copied to a new filename before inline jq reuse", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        ...
        "item": {
            "id": "cmd_payload",
            "type": "command_execution",
            "command": "cat <<'EOF' > entity-payload.json\\n{\\"type\\": \\"search/related\\", \\"item_type\\": \\"entity\\", \\"item_id\\": \\"light.a\\"}\\nEOF\\ncp entity-payload.json copied.json\\nha-nova relay ws --data-file copied.json --jq-file related-filter.jq --out related.json",
            "aggregated_output": "",
            "exit_code": 0,
            "status": "completed",
        },
...
`);
  ...
  it("fails when bulk review batches config reads through xargs -i", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
