...
`);
  ...
  it("allows one extra config read when the command item id is explicitly marked as related evidence", () => {
    const result = runPythonValidator(`
import importlib.util
import json
import sys
...
events = [
    ...
    },
    {
        "type": "item.completed",
        "item": {
            "id": "collision_related_config_read",
            "type": "command_execution",
            "command": "ha-nova relay core --method GET --path /api/config/automation/config/$extra_unique_id --jq-file config_filter.jq --out config.json",
            "aggregated_output": "ENTITY=automation.extra_overlap\\nUNIQUE_ID=999\\n{\\"id\\": \\"999\\"}",
            "exit_code": 0,
            "status": "completed",
        },
    },
