...
- Windows real-machine lane executed on the Win11 VM over SSH (2026-06-12): fresh 0.5.0 bundle install via the maintainer override, check-update correct, downgrade protection confirmed, real `setup gemini` + doctor against the live HA/Relay (token piped from the Linux host, test-keyring override per the documented desktop-validation pattern), standard uninstall with background handoff, recovery-marker contract, and full purge incl. PATH and marker cleanup.
- The lane caught two real Windows bugs, both fixed and proven live on the VM: (1) install.ps1 crashed under StrictMode on partial uninstall markers (optional fields like remaining_paths missing when the helper dies early) — all optional marker reads now go through Get-UninstallStatusField; (2) the uninstall helper failed PATH cleanup with "The filename or extension is too long" on real-world user PATHs (25K chars) because setWindowsUserPath passed the full value on the PowerShell command line — the value now travels via environment variable.
- Known Windows follow-up (not release-blocking, SSH-only): Credential Manager writes fail with a raw "A specified logon session does not exist" in network logon sessions; needs classification analogous to the Linux keyring error classes.
