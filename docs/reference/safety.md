...
| No behavioral or feature-use analytics. Normal release checks contact GitHub; the only additional measurement is the strictly opt-in, ID-free [census](census.md), which never sends without your explicit yes | `cli/census.go` opt-in guard (`if !state.Enabled`); known census endpoint/send symbols are confined to `cli/census*.go` | `scripts/check-docs.sh` checks [11] and [12] pin that guard and confinement and reject configured analytics-vendor patterns; `cli/census_test.go` (`TestCensusNeverSendsWithoutEnabled`) |
| The census payload is exactly `{schema, version, relay?, os}` — no ID, IP field, or client timestamp — and cannot grow silently; consent/timing state stays local in `census.json` | `cli/census.go` (`censusPayload`); `census-worker/src/census.ts` strict field allowlist; `cli/census_state.go` | `cli/census_test.go` (`TestCensusWirePayloadExactShape`); `tests/census-worker/worker.test.ts` cross-contract test ("matches the json tags of the Go payload struct exactly") |
