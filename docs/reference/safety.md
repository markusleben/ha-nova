...
| No behavioral or feature-use analytics. Normal release checks contact GitHub; the only additional measurement is the strictly opt-in [census with an identifier-free application JSON body](census.md), which never sends without your explicit yes. Cloudflare is the hosting provider for the census endpoint and processes source-IP and connection metadata for HTTPS | `cli/census.go` opt-in guard (`if !state.Enabled`); known census endpoint/send symbols are confined to `cli/census*.go` | `scripts/check-docs.sh` checks [11] and [12] pin that guard and confinement and reject configured analytics-vendor patterns; `cli/census_test.go` (`TestCensusNeverSendsWithoutEnabled`) |
| The census JSON body is exactly `{schema, version, relay?, os}` — no installation/device/user ID, IP field, or client timestamp — and cannot grow silently; consent/timing state stays local in `census.json`. HA NOVA Worker code does not read source-IP metadata, and application storage/public statistics do not store it | `cli/census.go` (`censusPayload`); `census-worker/src/census.ts` strict field allowlist; `census-worker/src/request-adapter.ts` positive Request-read allowlist; `cli/census_state.go` | `cli/census_test.go` (`TestCensusWirePayloadExactShape`); `tests/census-worker/worker.test.ts` cross-contract and Request-access tests; `scripts/check-docs.sh` check [12b] |
| A census ping never changes a byte of command output. While local census state remains intact, the client records at most one attempt per ISO week, asks the interactive question at most once, and shows the AI-client callout at most three times; successful opt-out waits for an in-flight request and prevents later sends. | `cli/census.go` (one locked stamp-and-send coordinator); `cli/census_state.go` (shared process/cross-process lock); `cli/census_ask.go` | `cli/census_concurrency_test.go`; `cli/census_test.go` (`TestCheckUpdateJSONOutputByteIdenticalWithCensus`, `TestCensusPingStampsWeekBeforeSendAtMostOncePerWeek`); `cli/census_ask_test.go` |
| No cloud relay for Home Assistant data: HA requests and responses go only between your machine and your Home Assistant. Release checks and an opted-in census carry only the metadata documented above | The relay only talks to `HA_URL` (`nova/src/ha/*`); it cannot proxy other hosts | Relay tests; `docs/reference/bridge-architecture.md`; [census contract](census.md) |
