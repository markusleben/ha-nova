...
| Guarantee | Enforced by | Verified by |
|---|---|---|
| Every endpoint requires authentication: the relay bearer token (standalone/legacy) or an authenticated paired device (App mode); the pairing endpoints themselves use short-lived, single-use codes. `/health` is never open | `nova/src/http/server.ts`; `nova/src/http/handlers/pair.ts`; `nova/src/http/handlers/pair-v1.ts`; `nova/src/http/handlers/device-auth.ts` | `tests/security/auth.test.ts`; `tests/http/pair.test.ts`; `tests/http/device-auth.test.ts`; the container smoke test asserts a 401 on `/health` |
