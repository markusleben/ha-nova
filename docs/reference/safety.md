...
| No telemetry, no analytics, no phone-home — the single exception is the strictly opt-in, ID-free [census](census.md), which never sends without your explicit yes | `cli/census.go` opt-in guard (`if !state.Enabled`); the send path is confined to `cli/census*.go` | `scripts/check-docs.sh` checks [11] and [12] fail the build on telemetry patterns anywhere else or a leaked send path; `cli/census_test.go` (`TestCensusNeverSendsWithoutEnabled`) |
| The census payload is exactly `{schema, version, relay?, os}` — no ID, no IP stored, no timestamps — and cannot grow silently | `cli/census.go` (`censusPayload`); `census-worker/src/census.ts` strict field allowlist | `cli/census_test.go` (`TestCensusWirePayloadExactShape`); `tests/census-worker/worker.test.ts` cross-contract test ("matches the json tags of the Go payload struct exactly") |
| A census ping never changes a byte of command output and fires at most once per ISO week; the interactive question is asked at most once, and the AI-client callout appears at most three times, then the question closes itself | `cli/census.go` (stamp-before-send week gate + exclusive week marker); `cli/census_ask.go` (asked-flag stamped before the prompt; 3-emission cap) | `cli/census_test.go` (`TestCheckUpdateJSONOutputByteIdenticalWithCensus`, `TestCensusPingStampsWeekBeforeSendAtMostOncePerWeek`); `cli/census_ask_test.go` (`TestCensusAskStampsAskedBeforePromptAbortedPromptNeverReasks`, `TestCensusSkillNoticeCapThreeEmissionsThenPermanentSilence`) |
