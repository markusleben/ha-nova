...
| Home Base status and the current pairing code are visible only through authenticated, admin-only Supervisor ingress; spoofed headers on the direct Relay port are rejected | `nova/config.yaml`; `nova/src/http/handlers/home.ts` | `tests/http/home.test.ts`; `scripts/check-docs.sh` check [9c] |
