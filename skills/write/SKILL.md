...
3. Suggestions + Pre-Write Checks (skip for `delete`):
   ...
   - **3d) Threshold Calibration (update only)**: when the update changes a `numeric_state` `above`/`below`, its `for:` duration, or a `wait_for_trigger`/`wait_template` timeout on a physical-process sensor, run the history-based preflight per `skills/ha-nova/threshold-calibration.md` and carry its findings (observed ranges, longest ambiguous phase, data gaps, both failure directions) into the preview — bounded reads of already-existing traces are the preflight's explicit exception to the no-auto-trace rule. Unrelated numeric edits skip it.
...
- `skills/ha-nova/threshold-calibration.md` — an update changes a physical-process threshold, `for:` duration, or wait timeout
