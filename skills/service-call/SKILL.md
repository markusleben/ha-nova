...
6. Verify result — match the check to what the call promises:
   ...
   - User-assisted proof: when verification needs the user to act physically (press the device's own button, operate it by hand), follow context skill → User-Assisted Readiness — read the baseline state and timestamp BEFORE the instruction, confirm ready, give one exact "act now" instruction, then re-read and compare.
