...
- Publishing to a device's `set`/command topic actuates real hardware. Preview it as an action, not as a message — and it takes the same typed `confirm:<token>`, because the broker path bypasses every HA-side gate the equivalent service call would apply.
