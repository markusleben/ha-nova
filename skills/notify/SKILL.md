...
5. Actionable buttons: the button press arrives as a `mobile_app_notification_action` EVENT in Home Assistant. This skill does not hold a long subscription for it, so the durable answer is an automation on `mobile_app_notification_action` (`ha-nova:write`). A bounded in-chat window can CATCH a tap, but cannot guarantee one: the relay is request/response, so the window opens after the send and Home Assistant does not replay `mobile_app_notification_action` — a press in that gap is simply gone. Offer it only when the user is standing at their phone, say how long you will wait, and never report an empty window as proof that nobody tapped; say the window may have missed it and fall back to the automation. This skill cannot wait longer than that window (the relay does not hold long subscriptions). To react to a press, the user needs an automation that listens for that event — hand off to `ha-nova:write` and explain the pattern instead of pretending to wait.
