...
Devices: HA NOVA exposes no device-delete capability. On Home Assistant 2026.8+ the legacy-named `config/device_registry/remove_config_entry` (deprecated; gone in 2027.9) and its admin-only successor `config/device_registry/remove` (2026.9+) remove the owning device and work only when the integration supports it. Report device orphans and route to the owning integration or the HA UI.
