...
- Item operations on ONE list — adds, completes, renames, updates, up to 10 — may confirm as a single grouped change set (`skills/ha-nova/grouped-change-set.md`): one manifest listing every item with its duplicate check and one natural confirmation — but read back after EACH applied operation, not once at the end. A provider that silently ignores the first update must stop the batch there, per the grouped contract's fail-fast ledger; a single trailing read would record the rest as applied anyway. Four items should not cost four rounds. List deletes keep `batch-safety.md` unchanged. One list per mutation; verify by reading back, not by service success alone.
