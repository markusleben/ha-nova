...
- retry exactly once only when verification shows no state change AND the call is a direct state-changing action whose read-back proves non-application — never for indirect or asynchronous runs (their effects are delayed), and never for disruptive or restart-class actions (`skills/ha-nova/outcome-verification.md`)
