...
5. Best-practice cache (automation domain only; skip for scripts):
   ...
   - Possible states: `fresh`, `stale`, `missing`, or `invalid` — `fresh` = `automation_bp_refreshed: true`, `automation_bp_refreshed_at` a parseable timestamp between now minus 30 days and now (a future timestamp is `invalid`, not fresh), `automation_bp_sources` a non-empty list, and `automation_bp_ha_version` a non-empty version string; older → `stale`; file absent → `missing`; everything else — unparsable, `automation_bp_refreshed` anything but boolean `true`, or any field absent, empty, or null → `invalid`
