...
- Template issues → https://www.home-assistant.io/docs/configuration/templating/ — only when a template check needs a function, filter, or constant signature `skills/review/checks.md` does not cover (fetch once per review; the checks themselves stay local and offline)
