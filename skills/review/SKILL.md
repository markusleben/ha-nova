...
1. If the user asks for a bulk audit by `prefix`, `domain`, `area`, or `label`, build a shortlist first using `skills/ha-nova/bulk-patterns.md`.
   ...
   - a narrow collision explanation may inspect a directly related target that is not part of the matched remainder when needed to classify a cluster; keep that read explicit, read-only, and outside the audited-item count
