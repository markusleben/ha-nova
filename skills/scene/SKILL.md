...
3. **Capture attributes deliberately** (the HA editor grabs only state + brightness for lights):
   ...
   - other domains: `state` plus clearly writable target attributes — cover `current_position` captures as `position`, climate `temperature`/`hvac_mode`, fan `percentage`/`preset_mode`, humidifier `humidity`/`mode`; never copy measurement or diagnostic attributes, and never the `current_*` twin of a setpoint (`current_temperature`, `current_humidity` are sensor readings)
