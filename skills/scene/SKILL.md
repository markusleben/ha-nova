...
- create, update, delete storage scenes (the scenes the HA scene editor stores in `scenes.yaml`)

Not in scope:
- activating scenes (`scene.turn_on`) — use `ha-nova:service-call`
- integration-owned scenes (Hue, deCONZ, ...) — see Editability Guard below
- `scene.create` runtime snapshots inside automations — those are transient constructs owned by `ha-nova:write` (see `skills/ha-nova/best-practices.md` → Persistence Model)
...
3. Present name, entities, and their captured target states.

### Create
1. Resolve every entity the scene should capture (exact `entity_id`; on ambiguity ask one blocking question). Read each entity's current state; warn when one is `unknown`/`unavailable` and leave it out unless the user insists.
2. Build the config body — `id` in the body MUST equal the id in the path; use the current epoch-milliseconds string as the new id (HA editor convention; POSIX example `date +%s000` — any unique numeric string works):
   ```json
...
3. **Capture attributes deliberately** (better than the HA editor, which grabs only state + brightness for lights):
   - light that is on: `state`, `brightness`, plus the active color attribute for its `color_mode` (`color_temp_kelvin` OR `rgb_color`/`hs_color`) — an off light exposes no color attributes, capture `state: "off"` only
...
Same as Create, but the entities map IS the live state: read each requested entity's current state and copy it (attribute rules from Create step 3). Tell the user which entities were captured with which values before confirming. For transient in-automation snapshots, `scene.create`/`snapshot_entities` stays with `ha-nova:write`.
...
4. POST the full merged body, then read back and verify both the intended change and the survival of unrelated entities.
...
3. Require exact token confirmation `confirm:<token>` — generate a short token, display it in the Options slot, and proceed only when the user types it back exactly (same tier as the other config-delete skills).
...
- Activation requests inside a scene task hand off to `ha-nova:service-call` — mention that `scene.turn_on` supports `transition` (lights only) and that `scene.apply` can apply a one-off state set without storing a scene.
