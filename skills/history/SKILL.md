...
- Measurement stats (temperature, humidity) carry `mean`/`min`/`max`. Metered `total_increasing` stats (energy, water, gas) carry `sum`/`state` — consumption over a window is `last sum − first sum`, NEVER an average (there is no `mean` there).
