...
# to 6500 for the secure device-pairing wave (relay 0.7): the OPAQUE (RFC 9807)
...
# NOVA owner console with CSRF-protected form actions).
...
if (( ACTUAL_LOC >= 1000 && ACTUAL_LOC <= 6500 )); then
  pass "src/ = ${ACTUAL_LOC} LOC (within 1000–6500 range)"
else
  fail "src/ = ${ACTUAL_LOC} LOC — outside the 1000–6500 range. If this is real growth, justify it and update the README claim in the release-prep PR."
fi
