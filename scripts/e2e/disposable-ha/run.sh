...
#   - the relay reaches Home Assistant over WebSocket (not just "the port is open")
...
#   - a REST proxy call returns real Home Assistant data
#   - a WS proxy call returns real Home Assistant data
...
if health.get("version") != expected_version:
    print(f"  FAIL: relay reports version {health.get('version')!r}, expected {expected_version!r} (baked-in version is what the CLI compatibility check reads)", file=sys.stderr)
    ok = False
else:
    print(f"  ok: relay reports its baked-in version ({expected_version})")
if health.get("ha_ws_connected") is not True:
    print("  FAIL: relay is not connected to Home Assistant over WebSocket", file=sys.stderr)
    ok = False
else:
    print("  ok: relay is connected to Home Assistant over WebSocket")
