...
echo "Note: Create a GitHub Release (v${NEW_VERSION}) with goreleaser to publish relay binaries."
echo "Next: npm install && npm test && git add version.json package.json package-lock.json .claude-plugin/plugin.json .claude-plugin/marketplace.json && git commit -m 'chore: bump skill version to $NEW_VERSION'"
