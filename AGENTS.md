...
- **Upstream drift check (decision 2026-08-05):** before every release cycle — and ad hoc when a monthly HA release lands — screen upstream deltas against skill-pinned surfaces: the HA monthly release post, the HA developer-blog breaking changes, and HACS releases since the last logged check. Fixed question: "which new/changed features touch surfaces our skills pin (update/backup/calendar/statistics/lovelace/config-entries/repairs/assist/service schemas, entity/device registry WS schemas, and the HACS WS command/lifecycle schemas pinned in `skills/hacs/hacs-commands.md`)?" Log every run — hits AND clean passes — dated and append-only in `docs/work/upstream-drift-log.md`; each skill-relevant delta becomes an issue or joins the running PR train. Rationale: HA 2026.7's frontend-only "Update all" changed batch-update semantics the updates skill had to model; release-cadence screening catches this class before users do.
