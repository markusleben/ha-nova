...
- **Release rehearsal gate:** before any tag/publish, run `bash scripts/release/verify-release-pipeline.sh`, then use the tag-first rehearsal — a maintainer pushes `vX.Y.Z-rcN` on the reviewed commit, `release.yml` publishes the prerelease (no workflow can self-create `v*` tags; the ruleset blocks the Actions token), verify the public install, then tag the final `vX.Y.Z` on that same commit. See `docs/releasing.md` → "Release Candidate Gate". Never reintroduce an auto-publish step in `release-candidate.yml`.
