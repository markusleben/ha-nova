...
- **Release rehearsal gate:** before any tag/publish, run `HA_NOVA_RELEASE_AUDIT_REQUIRE_BYPASS=1 bash scripts/release/verify-release-pipeline.sh` (strict mode, as a maintainer with admin `gh auth`), then use the tag-first rehearsal — a maintainer pushes `vX.Y.Z-rcN` on the reviewed commit, `release.yml` publishes the prerelease (no workflow can self-create `v*` tags; the ruleset blocks the Actions token), verify the public install, then tag the final `vX.Y.Z` on that same commit. See `docs/releasing.md` → "Release Candidate Gate". Never reintroduce an auto-publish step in `release-candidate.yml`.
