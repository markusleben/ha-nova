...
- Review until a pass returns no REAL defect after realness/severity triage; at most two passes of the SAME lens. The distinct mandated passes (self-review, local Codex diff review, pre-PR adversarial batch) each count separately. Remaining edge-case depth goes into the PR body as a known limit. "Until no further issues are found" is unreachable on spec-shaped work and was the same uncapped loop one layer down.
...
- Safe by default: `git status/diff/log`. Push only when the user asks — except the first push of a branch whose PR the user asked you to open, and on a branch with an open PR the user asked you to drive, where the checklist's push steps are pre-authorized for that PR. Never push to `main`.
...
- **README gate (enforced):** `readme-release-gate` fails any PR that touches `README.md` without bumping `version.json` in the same PR, unless a maintainer adds `readme-stable:approved` (corrections describing the CURRENT stable only). Unreleased feature claims collect in the active `docs/work/next-release-body.md` draft — created when the first unreleased claim appears, so between releases it does not exist. It is version-free by design; the release-prep PR renames it to `<version>-release-body.md` once it picks the number, which no earlier PR may presume.
...
- **Cloud evidence sessions run through the tool (since #570, decision 2026-08-13):** `bash scripts/release/build-cloud-evidence.sh <pr>` owns the mechanics — candidate dispatch/reuse bound by run-id fence + artifact identity (the bundle workflow's `run-name` interpolation is inert, see #574: runs are NOT findable by request_id or title), local checksum and pre-execution identity verification, per-platform provenance — and prints the envelope template. The check booleans are the maintainer's attestation: edit the template, record the ledger in the PR, then `--set --envelope <file>` validates against the live target and writes BOTH secret locations with verified timestamps. Right after the squash merge, run `--repoint <file>` to rename the envelope to the new `main` commit. Set `HA_NOVA_VERSION_TAG=v<skill_version>-rcN` (low rc numbers are taken). To re-evaluate `cloud-source-gate` after setting evidence, rerun the PR's CI run — never the broker run, which refuses re-evaluation within an attempt ("source check attempt already has a terminal result"). `docs/releasing.md` stays canonical for the manual reference path.
...
- **Release-bound review hardening:** do local/self review before opening the PR. After the PR exists, use the fast path only: after each relevant fix, run targeted local verification, push immediately, and trigger `@codex` immediately. After PR creation, Codex bot + CI are the review path; do not add extra SERIAL local review gates between a fix and its push. Two things are not "extra gates" and stay required: the finding-class sweep (it is part of producing the fix, not a review of it) and the parallel batch before the fifth trigger, which is mandatory (see Post-PR batch trigger).
...
- **Post-PR batch trigger (MANDATORY, decision 2026-08-10):** once FOUR `@codex` rounds on one PR have returned findings without a clean verdict, the batch runs BEFORE the fifth trigger — stop answering findings one at a time. Run 2-3 adversarial subagents IN PARALLEL over that PR's full current diff — distinct lenses (correctness/domain vocabulary, races and ordering, cross-file contract consistency), each briefed with the findings already fixed so they do not re-tread them — run what they return through the SAME realness/severity triage as the convergence cap below — the two rules govern different things, the batch decides how you FIND, the cap decides what you FIX — then fix everything that survives triage in ONE commit and trigger Codex once. This is not optional and not a judgement call: a bot round costs 10-15 minutes of latency and returns findings serially, while three agents return a round's worth of ground in one pass. Measured on #513/#527 (2026-08-09/10): ~40 rounds of serial fixing versus 30+ findings in a single parallel pass, including defects the serial loop had not reached in eight hours. Not automated on purpose: a hook that counted the trigger COMMANDS was built and dropped — it could not see a multi-line body (the normal shape of a real reply) while any command merely mentioning the pattern inflated the count, and an enforcement signal that is wrong in both directions is worse than none. Count the rounds from the PR's own review history.
...
- **Manifest-label rule:** if a PR changes `package.json`, `package-lock.json`, `nova/package.json`, or `nova/package-lock.json`, add `manifest-review:approved` before `@codex` / `gh pr checks --watch`, and only once the PR body states which manifest lines changed and why. The gate catches supply-chain drift; labelling by reflex makes it green by default.
...
- **PR Merge / Release Commit Gate — MANDATORY CHECKLIST (do NOT skip any step):**
  ...
  - [ ] 2b. If the PR changes `README.md` without bumping `version.json`, it fails `readme-release-gate` unless it is a stable-truth correction — then show the user the `README.md` diff and add the label only after they confirm it describes the CURRENT stable: `gh pr edit <nr> --add-label readme-stable:approved` (the gate only checks WHO labelled, and an agent runs as the maintainer — so the label is a real decision, not a formality) (feature/version claims belong in the release-body draft instead). Like 2a, the label is SHA-specific: re-apply it after any later commit on the PR.
  - [ ] 3. For the initial PR SHA and for every later relevant SHA: run targeted local verification only, push immediately if needed, then immediately trigger Codex review/re-review — once four rounds have returned findings, the Post-PR batch trigger runs first and this step waits for it: `gh pr comment <nr> --body "@codex"`.
  ...
  - [ ] 6. If findings OR any new relevant delta is introduced afterward → fix, run targeted verification, push immediately, then **trigger re-review** (once four rounds have returned findings, batch the fixes per the Post-PR batch trigger before pushing): `gh pr comment <nr> --body "@codex"` — pushes alone do NOT trigger re-review. Then go back to step 3 for the new SHA.
...
- In main: probably needed, ask user. This is a blocking question — the Autonomy Gate's "pick a default" step does not apply to it, because the wrong default breaks someone else's working setup.
...
- Subagent review hygiene (user requirement, amended 2026-08-10): Codex bot remains the final review instance for merge/tag clearance — a local pass never substitutes for it. What is banned is a SERIAL local gate between a fix and its push, which only adds latency. A PARALLEL adversarial batch is a different thing and is REQUIRED before the fifth trigger (see Post-PR batch trigger). The original blanket wording — "after PR creation, do not insert extra local subagent review gates" — is deleted: it forbade the one move that ends a long review loop, and it cost an eight-hour night on #513/#527.
- Review speed rule (user requirement): after each relevant push, trigger `@codex` immediately and let CI run; fix only real Codex/CI findings after the PR exists — the one exception is the Post-PR batch trigger's parallel subagent round, due before the fifth trigger.
