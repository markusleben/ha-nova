...
Version: 0.26 (2026-08-19) — bump this header on every local amendment.
...
- **Routine PRs: max ONE Codex round**, then merge on green required checks (`codex-review-gate` is advisory on `main`). **Release-bound:** one real bot verdict on the final head SHA (timeout is never a verdict).
- **Parallel batch (MANDATORY, release-bound only):** the first round returning >2 new real findings switches to batch mode — run 2-3 adversarial subagents in parallel over the full current diff (distinct lenses, briefed with what is already fixed), triage their output by the P1/P2/P3 rules, fix everything surviving in ONE commit, trigger once. Never close/reopen a PR to reset a review loop.
...
- [ ] 5. Bot signal: read the bot comment's own `Reviewed commit:` SHA and compare to `gh pr view <nr> --json headRefOid` — reactions persist across pushes and cannot be trusted per-SHA. Check reviews and inline comments with `--paginate`. Clean comment text: `Codex Review: Didn't find any major issues`.
- [ ] 6. Findings → triage per Review triage; fix, push, re-trigger (bare) for release-bound; routine merges after its one round. New relevant SHA → back to step 3.
...
- [ ] 7. Evidence (every product PR while Cloud remote is enabled): pick the next free tag via `git ls-remote --tags origin 'v<skill_version>-rc*'` + recent `gh run list --workflow cloud-candidate-bundle.yml`, then `HA_NOVA_VERSION_TAG=v<ver>-rcN bash scripts/release/build-cloud-evidence.sh <nr>` (darwin-local since #593), edit the template booleans you can attest, record the qualification ledger as a PR comment (carry/waiver form: copy the shape from #588's ledger; the risk-scope spec owns the invalidation map and the reference-smoke waiver), then `--set --envelope <file>` (writes BOTH secret locations; verify both `updated_at` are from this session).
...
- [ ] 9. Release-bound: confirm the head SHA still equals the SHA of the latest clean bot verdict; otherwise back to step 3.
...
- **Platform reality (2026-08-19):** `cloud_remote_platforms` is `["darwin"]` (#593); evidence provenance runs locally on macOS. The reference-smoke waiver (#592, risk-scope spec `docs/work/2026-07-30-cloud-release-evidence-risk-scope-spec.md`) covers real-platform reruns when validation infrastructure is unavailable — the envelope, commit/tree identity, per-platform provenance, and the live `installed_relay_app` read are never waivable. Never re-enable a platform without a host that can run its provenance. Manual RC lanes for a gone OS are never skipped silently: either the release carries no installer/onboarding delta for that OS (state it in the release PR) or record a dated accepted gap next to the ledger.
