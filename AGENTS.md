...
- **Platform reality (2026-08-28):** `cloud_remote_platforms` is `["darwin","linux","windows"]` again (v0.25.1 restore — the 2026-08-19 darwin-only scoping #593 rested on a stale premise; both lab hosts are SSH-reachable via VPN). Per-platform provenance is non-waivable and runs remotely (`HA_NOVA_LINUX_SSH`/`HA_NOVA_WINDOWS_SSH`); the reference-smoke waiver (#592) covers only smoke/qualification reruns. Never enable a platform whose provenance host is truly gone. Shrinking the platform list is a user-facing feature removal and ALWAYS a blocking user decision (Backwards Compat rule) — never an autonomous default. Manual RC lanes for an OS are never skipped silently: state the no-delta case in the release PR or record a dated accepted gap next to the ledger.
